...
export default function InternationalAnalysesPage({
  slug,
  title,
  emoji = "🌍",
}: {
...
function Header({ emoji, title }: { emoji: string; title: string }) {
  return (
    <div className="mb-6">
      <div className="inline-flex items-center gap-2 rounded-full bg-amber-400/10 px-3 py-1 text-[11px] uppercase tracking-wide text-amber-200">
        {emoji} {title.split(" — ")[0].toLowerCase()}
      </div>
      <h1 className="mt-2 text-3xl font-extrabold">{title}</h1>
      <div
        aria-hidden
        className="mt-3 h-10 w-full rounded-full bg-gradient-to-r from-amber-400/15 via-white/5 to-amber-400/15 blur-xl"
      />
    </div>
  );
}
  ...
  return (
    <div className="mb-6">
      <input
        ...
        className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder-white/50 outline-none focus:ring-2 focus:ring-amber-400"
      />
    </div>
  );
}

/* ---------- Carte d’analyse (multi-lignes + badges) ---------- */
    ...
    <article className="rounded-2xl border border-white/10 bg-white/5 p-5 shadow-[0_8px_30px_rgba(0,0,0,.35)]">
      <header className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {analyse.decouverte && (
            <span className="rounded-full bg-amber-400/20 text-amber-200 text-xs px-2 py-0.5">
              Découverte
            </span>
          )}
      ...
      <details className="mt-3 group">
        <summary className="cursor-pointer inline-flex items-center gap-2 text-sm text-amber-300 group-hover:text-amber-200">
          ▶ Lire l’analyse
        </summary>
